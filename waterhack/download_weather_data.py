# -*- coding: utf-8 -*-
"""
This script adapted from https://github.com/ekapope/WorldWeatherOnline

The main change is in function retrieve_this_location, where the number of attempts to
download the data is increased (N_attempt=100). For some reason the provider does not
...
hist_weather_data = download_weather_data.retrieve_hist_data(api_key,
                                location_list,
                                start_date,
                                end_date,
                                frequency,
...
def retrieve_this_location(
    ...
    list_mon_begin = pd.concat(
        [pd.Series(pd.to_datetime(start_date)), pd.Series(list_mon_begin)],
        ignore_index=True,
    )
    ...
    list_mon_end = pd.concat(
        [pd.Series(list_mon_end), pd.Series(pd.to_datetime(end_date))],
        ignore_index=True,
    )
